...
class EventTest(TestCase):
    ...
    def test_process_event(
        ...
        mock__create_from_stripe_object.assert_called_once_with(mock_data, stripe_account=None)
        (
    ...
    def test_process_event_failure_rolls_back(self, invoke_webhook_handlers_mock):
        ...
        create_from_stripe_object_mock.assert_called_once_with(event_data, stripe_account=None)
        self.assertFalse(
            Event.objects.filter(id=FAKE_EVENT_TRANSFER_CREATED["id"]).exists()
        )
